...
const HELP = `
project-hub - A TUI project manager

Usage:
  projecthub              Open interactive TUI
  projecthub add <path>   Add all projects inside a directory
  projecthub list         List all projects
  projecthub remove <id>  Remove a project by ID or name
  projecthub help         Show this help message
...
Examples:
  projecthub add ~/projects    Add all project directories inside ~/projects
  projecthub add .             Add all projects in current directory
`
...
export function runCli(args: string[]): boolean {
  ...
  if (!command || command === "" || command.startsWith("-")) {
    ...
    if (args.includes("-v") || args.includes("--version")) {
      console.log("project-hub v0.1.0")
      return true
    }
    // No command = run TUI
    return false
  ...
  switch (command) {
    ...
    case "add": {
      ...
      if (!path) {
        ...
        console.error("Usage: projecthub add <path>")
        process.exit(1)
      }
    ...
    case "list": {
      ...
      if (projects.length === 0) {
        ...
        console.log("Add some with: projecthub add <path>")
        return true
      }
    ...
    case "remove": {
      ...
      if (!idOrName) {
        ...
        console.error("Usage: projecthub remove <id|name>")
        process.exit(1)
      }
      try {
